...
var rotateCopy = function(arr, direction, times) {
  ...
  // set default value of times
  times = typeof times === 'undefined' ? 1 : times;

  var rotated = arr.map(function(val) { return val; });

  for (var i = 1; i <= times; i++) {
    direction === 0 ?
      rotated.push(rotated.shift(rotated[0])) :
      rotated.unshift(rotated.pop(rotated[0]));
  }
